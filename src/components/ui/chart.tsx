                    ...
                    {!hideIndicator && (
                      <span
                        className={`shrink-0 rounded-[2px] ${
                          ...
                          indicator === "dashed"
                            ? "w-0.5 border-dashed border-r-2"
                            : ""
                        }`}
...
>(
  (
    ...
    >
      {payload.map((item) => {
        const key = `${nameKey || item.dataKey || (item as any).name || "value"}`;
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
        if (!itemConfig) return null;
        return (
          <div key={item.value} className="flex items-center gap-1.5">            {!hideIcon && itemConfig.icon && <itemConfig.icon />}
            {itemConfig.label}
          </div>
        );
      })}
